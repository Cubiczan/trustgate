...
**Boundary (stated, per the row's scope):** the seal is application-level (database rows + recomputable chain), not an external attestation; a database-level adversary who can rewrite every row can rebuild a consistent chain. The row's deploy condition — independent of external trust level — is met by making any *partial* tampering (the realistic case: editing one denial record) detectable.

**Append atomicity:** the read-prevHash-then-create pair runs inside one `db.$transaction` at `Serializable` isolation with a bounded write-retry (`src/lib/appendWithRetry.ts`, used by `src/app/api/uipath/route.ts`). Under PostgreSQL production — whose default READ COMMITTED isolation would let two concurrent transactions both read the same `prevHash` and fork the chain — Serializable makes one of the concurrent writers abort with a P2034 conflict, and the bounded retry re-reads the fresh `prevHash` and re-appends (up to five attempts). On the SQLite dev database the isolation level is a no-op and the single-writer model serializes. The chain orders by monotonic `id` (createdAt has ms granularity and can tie under burst writes).

**On a break (`intact: false`):** the verifier's report locates the first inconsistent entry; treat that entry and everything after it as untrusted until remediated. Operator response: stop relying on the sealed export, notify the compliance owner, reconcile the flagged rows against the upstream source of truth, and rebuild by restoring and re-sealing from that source (the demo seed's deterministic path shows the mechanics). A break never self-heals and there is no automatic repair path.
